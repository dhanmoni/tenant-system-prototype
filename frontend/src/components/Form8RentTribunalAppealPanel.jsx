...
export default function Form8RentTribunalAppealPanel({ onBack }) {
	...
	const submit = async () => {
		...
		try {
			...
			setSuccess(data?.message || 'Form 8 submitted successfully.')
		} catch (err) {
			const msg =
				err?.response?.data?.message ||
				(err?.response?.data?.errors
					? Object.values(err.response.data.errors).flat().join('. ')
					: 'Failed to submit Form 8')
			setError(msg)
		} finally {
			setSubmitting(false)
		}
	...
	return (
		<div className="auth-card dashboard-card">
			<h1>Form 8: Appeal to be filed before the Rent Tribunal</h1>
			...
			>
				...
				<div className="form-actions">
					...
					<button type="submit" disabled={submitting}>
						{submitting ? 'Submitting...' : 'Submit Form 8'}
					</button>
				</div>
			</form>
		</div>
