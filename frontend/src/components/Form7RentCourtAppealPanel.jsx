...
export default function Form7RentCourtAppealPanel({ onBack }) {
	...
	const submit = async () => {
		...
		try {
			...
			setSuccess(data?.message || 'Form 7 submitted successfully.')
		} catch (err) {
			const msg =
				err?.response?.data?.message ||
				(err?.response?.data?.errors
					? Object.values(err.response.data.errors).flat().join('. ')
					: 'Failed to submit Form 7')
			setError(msg)
		} finally {
			setSubmitting(false)
		}
	...
	return (
		<div className="auth-card dashboard-card">
			<h1>Form 7: Appeal before the Rent Court</h1>
			...
			>
				...
				<div className="form-actions">
					...
					<button type="submit" disabled={submitting}>
						{submitting ? 'Submitting...' : 'Submit Form 7'}
					</button>
				</div>
			</form>
		</div>
