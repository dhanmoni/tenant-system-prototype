...
class TenancyApplicationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'property_tenancy_end_date' => ['required', 'date', 'after_or_equal:property_possession_date'],
    ...
    /**
     * Lookup a completed tenancy record by UIN for service-form auto-fill.
     */
    public function lookupByUid(Request $request)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
    ...
    public function update(Request $request, TenancyApplication $tenancyApplication)
    {
        ...
        $data = $request->validate([
            ...
            'landlord_email' => [$tenancyApplication->initiator_role === 'LANDLORD' ? 'required' : 'nullable', 'email'],
            'landlord_phone' => ['required', 'string', 'max:30'],
            ...
            'tenant_email' => [$tenancyApplication->initiator_role === 'TENANT' ? 'required' : 'nullable', 'email'],
            'tenant_phone' => ['required', 'string', 'max:30'],
    ...
    private function serializeTenancyForAutofill(TenancyApplication $application): array
    {
        return [
            'uid' => $application->uid,
            'application_no' => $application->application_no,
