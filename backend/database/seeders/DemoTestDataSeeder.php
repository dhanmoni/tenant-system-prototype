...
class DemoTestDataSeeder extends Seeder
{
    public function run()
    {
        ...
        // 2. Setup context
        $raAssistant = User::where('email', 'ra.assistant@nic.in')->first();
        $district = $raAssistant ? District::find($raAssistant->district_id) : District::first();
        if (!$district) return;
